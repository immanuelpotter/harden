...
pam_pw_lockouts(){
  grep_check "auth required pam_faillock.so preauth audit silent unlock_time=900" /etc/pam.d/password-auth 'auth required pam_faillock.so preauth audit silent=900'
...
pam_pw_remember(){
  grep_check "^password\s+sufficient\s+pam_unix.so" /etc/pam.d/password-auth 'password sufficient pam_unix.so remember=5'
  grep_check "^password\s+sufficient\s+pam_unix.so" /etc/pam.d/system-auth 'password sufficient pam_unix.so remember=5'
}

pam_pw_sha512(){
  grep_check "^password\s+sufficient\s+pam_unix.so" /etc/pam.d/password-auth 'password sufficient pam_unix.so sha512'
  grep_check "^password\s+sufficient\s+pam_unix.so" /etc/pam.d/system-auth 'password sufficient pam_unix.so sha512'
}


pam_su_restriction(){
  grep_check "pam_wheel.so" /etc/pam.d/su 'auth required pam_wheel.so use_uid'
}
